...
duplicates = member_df.exceptAll(member_df.dropDuplicates(['member_id']))
...
# 2. Find duplicates - simpler approach
duplicates = provider_df.exceptAll(provider_df.dropDuplicates(['provider_id']))
dup_count = duplicates.count()
print(f"⚠ Duplicates: {dup_count}")
if dup_count > 0:
...
# 2. Find duplicates - simpler approach
duplicates = claim_df.exceptAll(claim_df.dropDuplicates(['claim_id']))
dup_count = duplicates.count()
print(f"⚠ Duplicates: {dup_count}")
if dup_count > 0:
...
orphaned_claims = orphaned_member.unionByName(orphaned_provider).dropDuplicates(['claim_id'])
